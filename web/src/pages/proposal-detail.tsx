...
export function ProposalDetailPage() {
  ...
  const history = useProductHistory(data?.entity_key);
      ...
      {isError ? (
        ...
        <>
          <PageHeader
            ...
            description={`${data.entity_type.replace(/_/g, " ")} · ${data.field.replace(/_/g, " ")} · store ${data.store_number} · proposed ${formatDateTime(data.created_at)} by ${data.proposed_by}`}
            actions={
              <Button asChild variant="outline" size="sm">
                <Link to={`/data-review/products/${data.entity_key}`}>
          ...
          <div className="space-y-4">
                ...
                <CardContent className="flex flex-wrap items-center gap-x-4 gap-y-1 px-5 py-3 text-[0.8rem]">
                  ...
                  {data.resulting_mapping ? (
                    <span className="text-muted-foreground">
                      → <span className="font-mono">product_case_mappings.{data.resulting_mapping.item_code}</span>{" "}
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              ...
              <Card className="gap-0 p-0">
                <CardHeader className="px-5 py-3">
                  <CardTitle className="text-[0.9rem]">This product's history</CardTitle>
                  <p className="text-[0.75rem] text-muted-foreground">
                    Every proposal ever made for UPC {data.entity_key}, oldest first.
                  </p>
                </CardHeader>
                <CardContent className="px-5 pb-4">
                  {history.isError ? (
